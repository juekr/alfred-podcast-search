...
function format_single_for_alfred($item) {
	...
	$generator = empty($item["generator"]) ? "" : "".trim($item["generator"]);
	$explicit = (empty($item["explicit"]) or $item["explicit"] == false or $item["explicit"] == "false") ? "false" : "true";

	$frontmatter = array(
		"feed: ".$item["originalUrl"],
		"website: ".$item["link"],
		"cover: ".$item["image"],
		"author: ".$item["author"],
    ...
    return [
        ...
        "mods" => [
			...
			"shift" => [
				"valid" => true,
                "arg" => $markdown,
                "subtitle" => "Copy markdown summary of: ".$item["title"],
			]
        ]
    ];
}
...
- [x] added markdown export option
